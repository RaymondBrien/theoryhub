...
class TestDashboardBaseViews(TestCase):
    ...
    def test_dashboard_view_redirects_to_login_page_if_user_not_authenticated(self):  # noqa
    ...
    def test_UserNotes_redirects_to_login_if_not_authenticated(self):
        ...
        self.client.logout()
        response = self.client.get(reverse('user_notes'))
        self.assertRedirects(response, '/accounts/login/?next=/dashboard/notes/')  # noqa
...
class TestQuizNotes(TestCase):
    ...
    def setUp(self):
        ...
        self.client.login(username='testuser', password='testpassword')
        self.quiznote = QuizNote.objects.create(
        ...
        self.quiznote2 = QuizNote.objects.create(
            user=self.user2, note='Test note 2')
        self.quiznote2.save()
    ...
    def test_user_note_edit(self):
        ...
        response = self.client.post(reverse(
            'edit_note', args=(self.quiznote.id,)), {'note': 'Edited note'})
        self.assertEqual(
            QuizNote.objects.get(id=self.quiznote.id).note, 'Edited note')

    def test_user_note_deletion(self):
        """Verify quiz note instance can be deleted successfully"""
        self.client.login(username='testuser', password='testpassword')
        response = self.client.post(
            reverse('delete_note', args=(self.quiznote.id,)))
    ...
    def test_user_cannot_see_other_users_notes(self):
        ...
        self.client.login(username='testuser', password='testpassword')
        self.quiznote = QuizNote.objects.create(
            user=self.user, note='New Test Note')
        self.client.logout()
        self.client.login(username='testuser2', password='testpassword')
        response = self.client.get(reverse('user_notes'))
