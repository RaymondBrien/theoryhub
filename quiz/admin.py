...
class AnswerInline(admin.TabularInline):
    """
    Enables admin to assign 4 multiple-choice answer options
    to each question in single interface for ease of editing
    and quiz creation.
...
class QuestionAdmin(admin.ModelAdmin):
    ...
    **Context**
    ...
    def save_related(self, request, form, formsets, change):
        ...
        if not any(
            answer.correct == 1 for answer in form.instance.answers.exclude(pk=form.instance.pk)):  # noqa
            form.add_error(
                None, "At least one answer must be marked as correct for each question.")  # noqa
