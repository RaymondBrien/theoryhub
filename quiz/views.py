...
def single_quiz(request, slug):
    ...
    **Template**
    ...
    form = AnswerSelection(request.POST, quiz=quiz) # add quiz for correct quiz fields specific to this quiz instance 
    if request.method == "POST":
        if form.is_valid():
            score = 0
            # process all questions at once
            for question in quiz.questions.all():
                answer_id = form.cleaned_data[f'question_{question.id}'] # see AnswerSelection form
            ...
            form = AnswerSelection(quiz=quiz)
